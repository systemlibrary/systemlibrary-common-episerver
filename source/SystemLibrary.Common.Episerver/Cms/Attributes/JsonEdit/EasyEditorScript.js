/* Taken from https://github.com/im4aLL/easyeditor */
...
(function ($, document, window) {
    ...
    EasyEditor.prototype.utils = function () {
        ...
        if ($('.' + _this.randomString + '-bind').length > 0) {
            var bindData;
            $('html').on('click paste keyup', _this.elem, function () {
                var el = _this.elem;
                clearTimeout(bindData);
                bindData = setTimeout(function () {
                    var boundElement = $('.' + _this.randomString + '-bind');
                    boundElement.html($(el).html());
